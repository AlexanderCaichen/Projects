...
public class AugmentedStreetMapGraph extends StreetMapGraph {
     ...
     */
    //Note: resulting location should have neighbors (neighbors.length != 0)
    public long closest(double lon, double lat) {
        double x = projectToX(lon, lat); //still longitude
        double y = projectToY(lon, lat); //still latitude

        if (kd== null) {
            //Point(x, y) -> x=longitude, y=latitude
            //get node list, only take those with neighbors, make all remaining nodes into points.
            points = getAllNodes().stream().filter(s -> neighbors(s.id()).size()>0).map(nodee -> new Point(projectToX(nodee.lon(), nodee.lat()), projectToY(nodee.lon(), nodee.lat()), (Object) nodee.id())).collect(Collectors.toList());
            kd = new KDTree(points);
        }
//        if (points.contains(new Point(lon, lat))){
//            System.out.println(points);
//            return 0;
//        }
        Point nearest_loc = kd.nearest(x, y);
        Long name = (Long) nearest_loc.getName();
        if (neighbors(name).isEmpty()) {
            //shouldn't happen, as those with neighbors are removed.
            //probably unnecessary.
            System.out.println("Some lone points not removed");
            //continue;
        }
        return name;
    }
    ...
    /**
     * Useful for Part IV. Do not modify.
